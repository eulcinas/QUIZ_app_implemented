...
const selectAnswer = (e) => {
  ...
  if (questions.length > index + 1) {
    ...
    if (score / questions.length === 1) {
      resultElement.innerHTML = `Correct answers: ${score} from ${questions.length}. You're a CSS <i class="fas fa-star"></i>`;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  progressBar.style.borderColor = 'var(--css-color)';
  progressBarFull.style.backgroundColor = 'var(--css-color)';
  easyLevelButton.style.backgroundColor = 'var(--css-color)';
  mediumLevelButton.style.backgroundColor = 'var(--css-color)';
  hardLevelButton.style.backgroundColor = 'var(--css-color)';
  timer.style.color = 'var(--css-color)';
  spanTextEasy.classList.add('color-css');
  spanTextMedium.classList.add('color-css');
  spanTextHard.classList.add('color-css');

  const data = await (await fetch('../data/CSS_questions.json')).json();
  questions = [...data];
});
