...
//! selecting new p tag from html for question counter
...
const selectAnswer = (e) => {
  ...
  if (questions.length > index + 1) {
    ...
    if (score / questions.length === 1) {
      resultElement.innerHTML = `Correct answers: ${score} from ${questions.length}. You're an HTML <i class="fas fa-star"></i>`;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  progressBar.style.borderColor = 'var(--html-color)';
  progressBarFull.style.backgroundColor = 'var(--html-color)';
  easyLevelButton.style.backgroundColor = 'var(--html-color)';
  mediumLevelButton.style.backgroundColor = 'var(--html-color)';
  hardLevelButton.style.backgroundColor = 'var(--html-color)';
  timer.style.color = 'var(--html-color)';
  spanTextEasy.classList.add('color-html');
  spanTextMedium.classList.add('color-html');
  spanTextHard.classList.add('color-html');

  const data = await (await fetch('../data/HTML_questions.json')).json();
  questions = [...data];
});
